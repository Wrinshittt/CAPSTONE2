...
// - Cleaner, more modern info panel (typography, spacing, sections, chips)
// - Remove the capsule/pills section below (monthly/advance/floor/status pills)
// - Keep "View full room info" button pinned to the bottom of the screen
// - Keep hero arrows + 3-dot indicator
//
// Updates (requested):
// - Details card: remove Status row
// - Right/Next arrow: remove the box background (show only the arrow icon)
// - Left arrow: remove the box background (show only the arrow icon)
// - Left arrow should ONLY appear on the 2nd and 3rd image (index 1 and 2)
// - Below Details: add “Available rooms of the same landlord” section
//
// NOTE: No existing functions/logic were removed — only UI composition/styling,
// plus added fetching for "same landlord rooms" using rooms.landlord_id
// and now: first uploaded image per recommended room (room_images smallest sort_order).
//
// UPDATE (this request):
// - Center the Apartment name and address
// - Increase font size for name/address to be more visible (clear indicator)
// - Remove the "available/status" capsule next to the apartment name
// - Do NOT remove/change any existing functions/logic
//
// UPDATE (this request):
// - Remove the right next button on the LAST image.
// - Display the profile of the landlord who posted the room BELOW the divider of the apartment name.
// - Do NOT remove/change any existing functions/logic
//
// COLOR UPDATE (this request):
// - Match CODE 2 palette (AppBar color 0xFF04354B, light neutrals, blue accents)
//
// UPDATE (this request):
// - Change "Available rooms of the same landlord" to a shorter label
// - Add indicator of how many rooms are available
// - Do NOT remove/change any existing functions/logic
//
// UPDATE (this request):
// - Modify the "More rooms" label so it clearly says these rooms are posted by the same landlord
// - Do NOT remove/change any functions or logic
//
// FIX (this request):
// - “More from this landlord” should NOT show rooms that are not admin-approved yet.
// - Do NOT remove/change any functions/logic (only tighten the existing filtering).
//
// NEW (this request):
// - Make the MAIN image scrollable too (when user scrolls down, hero image goes up).
// - Do NOT change/remove any existing functions or logic (only UI composition).
//
// NEW (this request):
// - Limit “More from this landlord” list to only 3 items.
// - If there are more than 3 available, show a clickable “More” word.
// - Do NOT change/remove any existing functions/logic (only UI composition/state).
//
// NEW (this request):
// - Move the “More/Less” text button BELOW the list.
// - Do NOT change/remove any existing functions/logic (only UI composition).
//
// NEW (this request):
// - Add indicator that the text button "More Rooms" is clickable.
// - Do NOT change/remove any existing functions/logic (only UI composition).
//
// ✅ UPDATE (this request):
// - Remove the dropdown expand/collapse behavior for "More Rooms".
// - Instead, navigate to RoomsList.dart when user clicks "More Rooms".
// - Do NOT remove/change any existing functions/logic (only UI behavior).
...
class _LTourState extends State<LTour> with SingleTickerProviderStateMixin {
  ...
  static const Color _brand = Color(0xFF04354B); // app bar color in CODE 2
  ...
  static const Color _accentPrice = Color(0xFF2563EB);
  ...
  // ✅ UI-only: show full list toggle (kept as state, but no longer used for dropdown)
  bool _showAllSameLandlord = false;
  ...
  // Vertical bow
  static const double kCurveMaxDeg = 8.0;
  static const double kCurvePower = 1.25;
    ...
    try {
      ...
      final changed = newImages.length != _images.length ||
          newImages.asMap().entries.any(
                (e) =>
                    _images.length <= e.key ||
                    _images[e.key].url != e.value.url,
              );
  ...
  Future<void> _reloadRoomInfo() async {
    try {
      ...
      if (room != null) {
        newLandlordId = room['landlord_id']?.toString();
      }
  ...
  void _openLandlordProfile() {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => Adminprofile(lid)),
    );
  }
  ...
  Future<void> _reloadSameLandlordRooms() async {
    ...
    try {
      ...
      for (final r in list) {
        ...
        final av =
            (r['availability_status'] ?? '').toString().toLowerCase().trim();
      ...
      if (filtered.isNotEmpty) {
        ...
        if (ids.isNotEmpty) {
          ...
          for (final im in (imgs as List? ?? const <dynamic>[])) {
            ...
            if (current == null ||
                so < ((current['sort_order'] as int?) ?? (1 << 30))) {
              firstByRoom[rid] = {
      ...
      setState(() {
        ...
        if (_sameLandlordRooms.length <= 3) {
          _showAllSameLandlord = false;
        }
      });
    } catch (e) {
      if (!mounted) return;
  ...
  void _openOtherRoom(String roomId) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => LTour(
  ...
  // ✅ NEW: Navigate to RoomsList.dart when "More Rooms" is clicked.
  void _openRoomsList() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const RoomsList()),
    );
  }

  /* ----------------------------- UI ----------------------------- */

  Widget _heroNavIconOnly({
  ...
  Widget _landlordPreviewCard() {
    ...
    final name = (_landlordDisplayName ?? 'Landlord').trim().isEmpty
        ? 'Landlord'
        : _landlordDisplayName!.trim();
                ...
                children: [
                      ...
                      if (_landlordVerified) ...[
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border:
                                Border.all(color: _chipText.withOpacity(0.30)),
                          ),
                          child: const Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Icon(Icons.verified, size: 14, color: _chipText),
                              SizedBox(width: 5),
                              Text(
                                'Verified',
                                style: TextStyle(
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          (_landlordEmail ?? 'Tap to view profile'),
                          style: const TextStyle(
  ...
  Widget _sameLandlordSection() {
    ...
    // ✅ Still show only top 3 items here (as before)
  ...
  Widget build(BuildContext context) {
    ...
    final displayName = rawName.isEmpty ? 'Apartment' : rawName;

    final rawLocation = (_address ?? widget.addressHint ?? '').trim();
    final displayLocation = rawLocation.isEmpty ? '—' : rawLocation;

    // Left arrow should ONLY show on 2nd and 3rd image (index 1 and 2).
    final showLeftArrow =
        _images.length > 1 && (_currentIndex == 1 || _currentIndex == 2);

    // Remove the right arrow on the LAST image.
    final isLastImage = _images.isNotEmpty && _currentIndex >= _images.length - 1;
        ...
        child: _loading
            ...
            : (_error != null
                ...
                : SingleChildScrollView(
                    // ✅ IMPORTANT: no extra bottom padding needed anymore
                    child: Column(
                      children: [
                        // ---------------- HERO PANORAMA ----------------
                        SizedBox(
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                // ✅ Replace dropdown with navigation button
                                ...
                                // ✅ MOVED HERE: Button is now part of the scroll content (bottom)
                                SafeArea(
                                  top: false,
                                  child: Padding(
                                    padding:
