// lib/TENANT/TAPARTMENT.dart
import 'dart:async';
import 'package:flutter/material.dart';
import 'package:supabase_flutter/supabase_flutter.dart';

// 👉 Open the tenant tour page on card tap
...
import 'TGMAP.dart'; // <-- Map screen
import 'package:smart_finder/TENANT/TBOTTOMNAV.dart';

// ====== SCORING HELPERS (GLOBAL) =======================================

// (Legacy) Function to compute Matching Score for bool/int preferences
// NOTE: Not used in this screen anymore but kept for compatibility.
double computeMatchingScore(
  Map<String, dynamic> tenantPrefs,
  Map<String, dynamic> landlordPrefs,
  Map<String, int> prefScores,
) {
  int obtainedScore = 0;
  int maxScore = prefScores.values.reduce((a, b) => a + b);

  tenantPrefs.forEach((key, tenantValue) {
    var landlordValue = landlordPrefs[key];

    bool isMatch = false;

    if (tenantValue is bool && landlordValue is bool) {
      isMatch = (tenantValue == landlordValue);
    } else if (tenantValue is int && landlordValue is int) {
      isMatch = (tenantValue == landlordValue);
    }

    if (isMatch) {
      obtainedScore += prefScores[key] ?? 0;
    }
  });

  return maxScore == 0 ? 0.0 : obtainedScore / maxScore;
}

// Matching Score (general weighted form):
...
double computeMatchingScoreExact(
  ...
  weights.forEach((key, w) {
    final m = matchFlags[key] ?? 0.0; // 1.0 if match, 0.0 otherwise
    weightedSum += w * m;
    totalWeight += w;
  });
...
class _TenantApartmentState extends State<TenantApartment> {
  ...
  // bottom nav index (0 = Apartment)
  int _selectedNavIndex = TenantNavIndex.apartment;
  ...
  // ===== Pretty hashtag maps (optional UI sugar)
  ...
  // ---------- Simple modal helper (auto-dismiss "toast modal") ----------
  ...
  // ✅ FIXED Availability filter (previous code treated "not available" as available)
  ...
  Future<void> _fetchRooms() async {
    ...
    try {
      ...
      final query = _sb
          ...
          .order('sort_order',
              referencedTable: 'room_images', ascending: true)
          .limit(200);
      ...
      for (final raw in data) {
        if (!_isApprovedStatus(raw['status']) || !_passesLandlordFilter(raw)) {
          continue;
        }

        if (!_isAvailable(raw['availability_status'])) {
          continue;
        }
        ...
        final String title =
            ((raw['apartment_name'] ?? '').toString().trim().isNotEmpty)
                ? (raw['apartment_name'] as String)
                : 'Apartment';

        final String address = (raw['location'] ?? '—').toString();

        // ✅ Ensure monthly is a numeric double
        final double monthly = _toDouble(raw['monthly_payment']);
        ...
        String? thumb;
        final imgs = (raw['room_images'] as List?) ?? const [];
        if (imgs.isNotEmpty) {
        ...
        final prefsArr = (raw['room_preferences'] as List?) ?? const [];
        ...
        final incArr = (raw['room_inclusions'] as List?) ?? const [];
        final incSet = <String>{};
  ...
  // ---------- Matching / Ranking ----------
  bool _hasPref(_RoomItem r, String label) =>
      r.prefLabels.contains(_norm(label));
  bool _hasInc(_RoomItem r, String label) =>
      r.incLabels.contains(_norm(label));
  ...
  ({
    ...
    final double matchingScore = computeMatchingScoreExact(
      criterionWeights,
      criterionMatches,
    );
    ...
    // ---------- Prioritization (Urgency + Demand) ----------
    // Demand: based on heartCount, capped.
    final demand = (r.heartCount / 20.0).clamp(0.0, 1.0);

    // Urgency: cheaper -> higher urgency score
    ...
    final priorityScore = computePriorityScore(
      factorValues,
      factorWeights,
    );
  ...
  void _applyRanking(List<_RoomItem> rooms) {
    ...
    // ✅ FIXED: stable + fair tie-breaking
    // Order:
    // 1) finalScore DESC
    // 2) matchingScore DESC
    // 3) prioritizationScore DESC
    // 4) monthly ASC (cheaper first)
    // 5) createdAt DESC (newer first)
    // 6) id ASC (final stable tie-break)
    rooms.sort((a, b) {
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Slider(
                                            ...
                                            max: 3,
                                            divisions: 2,
                                            value: (preferenceImportance[key] ??
                                                    1)
                                                .clamp(1, 3)
                                                .toDouble(),
                                            onChanged: isActive
                        ...
                        child: ElevatedButton(
                          ...
                          onPressed: () {
                            setState(() {
                              ...
                              _showAllWithFinalScore = false; // start in 70% mode
  ...
  void toggleFavorite(String roomId) {
    final user = _sb.auth.currentUser;
    if (user == null) {
      _showInfoDialog(context, 'You need to be logged in to heart.');
      return;
    }
  ...
  void toggleBookmark(BuildContext context, String roomId) {
    final user = _sb.auth.currentUser;
    if (user == null) {
      _showInfoDialog(context, 'You need to be logged in to bookmark.');
      return;
    }
  ...
  // ✅ UPDATED RULES:
  // - "More Apartment" only appears when filters are applied.
  // - Filter mode starts at 70% (Apply Filters).
  // - Clicking More Apartment switches to 50% mode.
  // - In 50% mode, the footer becomes "Back to 70%".
  Widget _moreApartmentsFooter() {
    if (!_filtersApplied) return const SizedBox.shrink(); // ✅ only show in filter mode

    final isMoreMode = _showAllWithFinalScore; // true => 50% mode
      ...
      child: Row(
        children: [
          ...
          InkWell(
            ...
            onTap: () {
              setState(() {
                // Toggle between 70% and 50% views while staying in filter mode
              ...
              _showInfoDialog(context, toastMsg);
            },
            child: Padding(
  ...
  Widget build(BuildContext context) {
    ...
    // - No filter: normal list + search + carousel + pagination
    // - Filter applied (default): show Final >= 70%
    // - More Apartment clicked: show Final >= 50%
                      ...
                      child: _filtersApplied
                          ? Column(
                              children: [
                                ...
                                // ✅ EMPTY STATE still shows footer (so user can switch 70% <-> 50%)
                                if (ranked.isEmpty) ...[
                                  Center(
                                    child: Padding(
                                      padding: const EdgeInsets.symmetric(
                                  ...
                                  ListView.separated(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      final card = TenantApartmentCard(
                                        title: item.title,
                                        address: item.address,
                                        priceText:
...
// ✅ Recommended card UI UPDATED to accept timerText
...
// -------- TenantApartmentCard and _RoomItem --------
